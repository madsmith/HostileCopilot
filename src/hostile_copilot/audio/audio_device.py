...
class AudioDevice:
    ...
    def _filter_frame(self, mic_frame: AudioData) -> AudioData:
        ...
        frames: np.ndarray[np.int32] = mic_frame.as_array().astype(np.int32) # type: ignore
        ...
        np_mic: np.ndarray[np.float32] = mic_frame.as_array(np.float32) # type: ignore
        np_playback: np.ndarray[np.float32] = np_playback.astype(np.float32) # type: ignore
        ...
        np_mic = np_mic[:min_length] # type: ignore
    ...
    def play(self, audio_data: AudioData | bytes):
        assert self.running, "AudioDevice is not running"
